...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    return programmersList[idN - 1].name
    // Leave this for last!
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: idN > 0 ? 'gold' : 'royalblue' // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setIdN(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          idN > 0 ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : 'Pick an awesome programmer'
        }
      </div>
    </div>
  );
